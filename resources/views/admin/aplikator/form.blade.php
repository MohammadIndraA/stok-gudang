@extends('layouts.app', ['title' => 'Form Aplikator'])
@section('styles')
    <!-- Css -->
    <link rel="stylesheet" href="{{ asset('design-system/assets/libs/vanillajs-datepicker/css/datepicker.min.css') }}">
@endsection
@section('content')
    <form action="{{ isset($aplikator) ? route('admin.aplikator.update', $aplikator->id) : route('admin.aplikator.store') }}"
        ...
        <div class="grid grid-cols-12 sm:grid-cols-12 md:grid-cols-12 lg:grid-cols-12 xl:grid-cols-12 gap-4 justify-between">
            <div
                ...
                <div class="w-full relative p-4">
                    {{-- tanggal  --}}
                    <x-input-v2 name="tanggal_lahir" label="Tanggal Lahir" :value="old('tanggal_lahir', $aplikator->tanggal_lahir ?? '')" placeholder="" required="true"
                        type="text" />
                    {{-- email --}}
                    <x-input-v2 name="email" label="Email" :value="old('email', $aplikator->email ?? '')" placeholder="Your email" required="true"
                        type="text" />
                    {{-- Tempat Lahir --}}
                    <x-input-v2 name="no_hp" label="No HP" :value="old('no_hp', $aplikator->no_hp ?? '')" placeholder="Tempat Lahir" required="true"
                        type="text" />
                    {{-- Tempat Lahir --}}
                    <x-input-v2 name="agama" label="Agama" :value="old('agama', $aplikator->agama ?? '')" placeholder="Tempat Lahir" required="true"
                        type="text" />
                    <!-- Password -->
                    ...
                    <div class="mb-3">
                        ...
                        <div class="flex gap-4 mt-1">
                            @php $jenis_kelamin = old('jenis_kelamin', $aplikator->jenis_kelamin ?? 'Laki-laki'); @endphp
            ...
            <div
                ...
                <div class="w-full relative mb-4">
                    <div class="flex-auto p-0 sm:p-4">
                        ...
                        {{-- nama lengkap --}}
                        <x-input-v2 name="nama_lengkap" label="Nama Lengkap" :value="old('nama_lengkap', $aplikator->nama_lengkap ?? '')"
                            placeholder="Masukan Nama Lengkap" required="true" type="text" />
                        {{-- Tempat Lahir --}}
                        <x-input-v2 name="tempat_lahir" label="Tempat Lahir" :value="old('tempat_lahir', $aplikator->tempat_lahir ?? '')" placeholder="Tempat Lahir"
                            required="true" type="text" />
                        <!-- status_perkawinan Radio -->
                        <div class="mb-3">
                            <label class="font-semibold text-neutral-600 text-sm">Status Perkawinan</label>
                            <div class="flex gap-4 mt-1">
                                @php $status_perkawinan = old('status_perkawinan', $aplikator->status_perkawinan ?? 'Belum Menikah'); @endphp
                                <label class="flex items-center gap-2">
                        ...
                        {{-- alamat --}}
                        <x-textarea name="alamat_lengkap" label="Alamat Lengkap" :value="$aplikator->alamat_lengkap ?? ''" :rows="4"
                            :required="true" />
