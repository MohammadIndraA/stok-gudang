...
@section('content')
    <div
        ...
        <div class="text-center p-6 bg-slate-900 rounded-t">
            <a href="index.html"><img src="{{ asset('design-system/assets/images/logo-sic.png') }}" alt=""
                    class="w-14 h-14 mx-auto mb-2"></a>
            <h3 class="font-semibold text-white text-xl mb-1">Let's Get Started IS-3</h3>
            <p class="text-xs text-slate-400">Sign in to continue to IS-3.</p>
        </div>
        @if (session('error'))
        ...
        <form class="p-6" action="{{ route('register') }}" method="POST">
            @csrf
            <div>
                <label for="name" class="font-medium text-sm text-slate-600 dark:text-slate-400">Full Name</label>
                <input type="name" id="name" name="name"
                    class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-2 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-primary-500 dark:focus:border-primary-500  dark:hover:border-slate-700  @error('name') border-red-500 @enderror"
            ...
            <div class="mt-4">
                <label for="password" class="font-medium text-sm text-slate-600 dark:text-slate-400">Password
                    Confirmation</label>
        ...
        <p class="mb-5 text-sm font-medium text-center text-slate-500"> Already have an account ? <a href="/login"
                class="font-medium text-brand-500 hover:underline">Sign In</a>
        </p>
    </div>
@endsection
